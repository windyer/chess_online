...
class IapppayForm(forms.Form):
    token = forms.CharField(max_length=200, required=True)
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        super(IapppayForm, self).__init__(*args, **kwargs)

    def _form_validate(self):
        if self.cleaned_data['app_version'] == '0.9':
            return
        data = u'{0}{1}{2}{3}{4}{5}{6}{7}'.format(
                    self.cleaned_data['token'], self.cleaned_data['device_id'], 
        ...
        if digest.hexdigest() !=  sign:
            raise IApppayError.AUTH_FAILED(token=self.cleaned_data['token'], msg='sign not the same')
    ...
    def clean(self):
        ...
        if settings.IAPPPAY.need_validate_form:
            self._form_validate()
        ...
        logger.debug("[token|%s] [login_ip|%s]", cleaned_data['token'], cleaned_data['login_ip'])

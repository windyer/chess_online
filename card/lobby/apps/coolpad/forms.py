...
class CoolpadForm(forms.Form):
    #user_name = forms.CharField(max_length=100, required=True)
    sessionId = forms.CharField(max_length=50, required=True)
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        super(CoolpadForm, self).__init__(*args, **kwargs)

    def _form_validate(self):
        if self.cleaned_data['app_version'] == '0.9':
            return
        data = u'{0}{1}{2}{3}{4}{5}{6}{7}'.format(
                    self.cleaned_data['sessionId'], self.cleaned_data['device_id'],

...
from card.lobby.apps.coolpad.service import CoolpadService
from card.lobby.apps.coolpad import serializers
import operator
import urllib
import ujson
@api_view(('GET',))
...
class Login(generics.CreateAPIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            service = CoolpadService(self.request.service_repositories, self.request.activity_repository)
...
class ChargeNotify(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            resp = service.process_charge_order(transdata, sign, signtype)

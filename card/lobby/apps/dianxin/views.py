...
from card.lobby.apps.dianxin.service import DianxinService
from card.lobby.apps.dianxin import serializers
import operator
import urllib
import ujson
@api_view(('GET',))
...
class Login(generics.CreateAPIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            service = DianxinService(self.request.service_repositories, self.request.activity_repository)
...
class ChargeNotify(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            service = DianxinService(self.request.service_repositories, self.request.activity_repository)
            resp = service.process_charge_order(**serializer.data)

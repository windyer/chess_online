...
from card.core.error.lobby.yuyang_error import YuYangError
...
from card.lobby.apps.yuyang import models as yuyang_models
...
class YuYangService(ViewService):

    def __init__(self, service_repositories, activity_repository):
        super(YuYangService, self).__init__(service_repositories, activity_repository)
    ...
    def _get_purchase_item(self, user_id, item_id):
        if item_id in ITEMS.coins:
            ...
            raise YuYangError.item_not_exist(user_id=user_id, item_id=item_id)
        return item
    ...
    def _create_cp_order(self, user_id, item_id, item, channel_id, app_id):
        ...
        charge_log = yuyang_models.YuyangLog()
        charge_log.cporderid = order_id
        charge_log.appuserid = user_id
        charge_log.appid = app_id
        charge_log.waresid = settings.YUYANG.yuyang_items[item_id].pay_point_num
    ...
    def check_charge_order(self, transid):
        app_id = settings.YUYANG.app_id
        app_key = settings.YUYANG.app_key
        ...
        query_url = settings.YUYANG.query_order_url
    ...
    @trace_service
    def process_charge_order(self, game_id,cp_trade_no, uid, total_fee,pay_status,sign):

        game_secure_key=settings.YUYANG.app_key
        s_sign = hashlib.md5(uid + cp_trade_no + total_fee +game_secure_key).hexdigest()
        if sign != s_sign:
            self.logger.debug(
                '[yuyang_charge] [orderid|%s] [amount|%s]  [sign|%s] [s_sign|%s]  not match',
                cp_trade_no, total_fee,  sign, s_sign)
            return {'status':'failed', 'desc':'sign wrong'}
        try:
            charge_log = yuyang_models.YuyangLog.get_yuyang_log_by_transid(cp_trade_no)
            if charge_log.result == 1:
                self.logger.debug(
                    '[yuyang_charge] [orderid|%s] [amount|%s]  [other_id|%s] already exist',
        ...
        yuyang_amount = int(total_fee.strip())
        if item is None or item.price  > yuyang_amount:
        ...
        check_stat = self.check_charge_order(cp_trade_no)
        if check_stat == -1:
            self.logger.debug(
                '[yuyang_charge] [orderid|%s] [amount|%s]  [other_id|%s] not exist',
                cp_trade_no, total_fee, charge_log.id)
            return {'status': 'failed', 'desc': 'order not exist'}
        if check_stat == -2:
            self.logger.debug(
                '[yuyang_charge] [orderid|%s] [amount|%s]  [other_id|%s] not pay',
                cp_trade_no, total_fee, charge_log.id)
            return {'status': 'failed', 'desc': 'order not pay'}
        ...
        store_service.charge(user_id, org_id, 1, charge_log.channel_id, 'charge', cp_trade_no,
                             revenue=int(yuyang_amount ))

        charge_status = ChargeStatus()
        charge_status.charge(user_id, item_id)

        charge_log.money = yuyang_amount
        charge_log.result = 1
        ...
        try:
            ...
            player_charge.charge_money = money + yuyang_amount
            player_charge.save()
            ...
            new_player_charge.charge_money = yuyang_amount
    ...
    def create_charge_order(self, user_id, channel_id, item_id):
        ...
        if item_id in ITEMS.same_items:
            item_id = ITEMS.same_items[item_id].item_id
        if item_id not in settings.YUYANG.yuyang_items:
            raise YuYangError.item_not_exist(user_id=user_id, item_id=item_id)
        item = self._get_purchase_item(user_id, item_id)
        ...
        if not charge_status.can_charge(user_id, profile.vip_title, item_id):
            raise YuYangError.item_has_charged(user_id=user_id, item_id=item_id)
        ...
        return {'transid':charge_log.transid,'price':item.price, 'name':item.name}

...
from card.core.error.lobby import YuYangError,HolyTreeError
...
from card.lobby.apps.yuyang.models import Yuyang

@go.logging.class_wrapper
class YuyangBackend(BaseBackend):
    ...
    def _login(self, uid):
        ...
        '''
        yuyang_user_id = uid
        self.logger.debug("[uid|%s]", yuyang_user_id)
        return yuyang_user_id
    ...
    def authenticate(self, user_name, sessionId, device_name, device_id, device_model,os_version, os_platform, app_version, channel, vender, login_ip, package_type,service_repositories, activity_repository, counter_repository, need_creation, **_):
        if channel != settings.YUYANG.channel:
        ...
        super(YuyangBackend, self).authenticate(package_type, device_id, app_version)
        ...
        try:
            yuyang = Yuyang.get_yuyang_by_uid(uid=uid)
            user_id = yuyang.user_id
        except YuYangError.UID_NOT_EXIST:
        ...
        self.logger.debug('yuyang [user_id|%s] [device_id|%s] [login_ip|%s]'
    ...
    def create_new_player(self, uid, device_id, nick_name, app_version,
                          channel, vender, login_ip, package_type, os_version, os_platform,
                          service_repositories, activity_repository):
        holytree_models.CreatePolicy.create_account("yuyang" + str(uid))
        try:
            player = super(YuyangBackend, self).create_new_player(
            ...
            holytree_models.CreatePolicy.delete_account("yuyang" + str(uid))
            raise

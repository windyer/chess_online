...
from card.lobby.apps.youku import serializers
import operator
import urllib
import ujson
@api_view(('GET',))
...
class Login(generics.CreateAPIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            service = YoukuService(self.request.service_repositories, self.request.activity_repository)
...
class ChargeNotify(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            service = YoukuService(self.request.service_repositories, self.request.activity_repository)
            resp = service.process_charge_order(orderid,price, passthrough, sign,uid)

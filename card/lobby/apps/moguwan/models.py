...
from card.lobby.aop import cache_moguwan_by_user_id, cache_moguwan_by_uid, args_limit
from card.core.error.lobby import MoguwanError

@go.logging.class_wrapper
class Moguwan(models.Model):
    ...
    @staticmethod
    @cache_moguwan_by_uid
    @args_limit('uid',)
    def get_moguwan_by_uid(uid):
        try:
            return Moguwan.objects.get(uid=uid)
        except (ObjectDoesNotExist):
            raise MoguwanError.UID_NOT_EXIST(uid=uid)
        except MultipleObjectsReturned:
            assert False, "MultipleObjectsReturned for [uid|%d]" %uid
            raise
        except Exception as ex:
            Moguwan.logger.exception(ex)
            raise

    @staticmethod
    @cache_moguwan_by_user_id
    @args_limit('user_id',)
    def get_moguwan_by_user_id(user_id):
        try:
            return Moguwan.objects.get(pk=user_id)
        except (ObjectDoesNotExist):
            raise MoguwanError.USER_ID_NOT_EXIST(user_id=user_id)
        except MultipleObjectsReturned:
    ...
    class Meta:
        db_table = 'player_moguwan'
        verbose_name = verbose_name_plural = ugettext('moguwan')


@go.logging.class_wrapper
class MoguwanLog(models.Model):
    ...
    @staticmethod
    def get_moguwan_log_by_cporderid(cporderid):
        try:
            return MoguwanLog.objects.get(cporderid=cporderid)
        except (ObjectDoesNotExist):
            return None
        except MultipleObjectsReturned as ex:
            MoguwanLog.logger.exception(ex)
            assert False, "MultipleObjectsReturned for get_moguwan_log_by_transid %s" % cporderid
            raise
        except Exception as ex:
            MoguwanLog.logger.exception(ex)
            raise
    ...
    def get_by_id(_id):
        try:
            return MoguwanLog.objects.get(id=_id)
        except (ObjectDoesNotExist):
            return None
        except MultipleObjectsReturned as ex:
            MoguwanLog.logger.exception(ex)
    ...
    class Meta:
        db_table = 'moguwan_charge_log'
        verbose_name = verbose_name_plural = ugettext('moguwan_charge_log')

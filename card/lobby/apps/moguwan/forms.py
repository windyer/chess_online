...
class MoguwanForm(forms.Form):
    access_token = forms.CharField(max_length=200, required=True)
    user_type = forms.CharField(max_length=200, required=True)
    uid = forms.CharField(max_length=200, required=True)
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        super(MoguwanForm, self).__init__(*args, **kwargs)

    def _form_validate(self):
        if self.cleaned_data['app_version'] == '0.9':
            return
        data = u'{0}{1}{2}{3}{4}{5}{6}{7}'.format(
                    self.cleaned_data['access_token'], self.cleaned_data['device_id'],
                    self.cleaned_data['device_model'], 
        ...
        if digest.hexdigest() !=  sign:
            ...
            raise MoguwanError.AUTH_FAILED(token=self.cleaned_data['access_token'], msg='sign not the same')
    ...
    def clean(self):
        ...
        if settings.BAIDU.need_validate_form:
            self._form_validate()
        ...
        logger.debug("[token|%s] [login_ip|%s]", cleaned_data['access_token'], cleaned_data['login_ip'])
